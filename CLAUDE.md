...
- `tilfluktsrom` - Shelter registry. Key columns: `id` (text), `alias`, `kundenavn`, `matrikkel`, `adresse` (Json), `bruksareal`, `byggeaar`, `forskrift`, `fredsbruk`, `har_aggregat`, `konstruksjon`, `plasser`, `vannklosett`. Note: no `title`, `kommune_nr`, `gnr`, `bnr`, `current_status`.
...
- `kunder` - Customers/owners. Columns: `id` (bigint), `aktiv`, `navn`, `nummer`, `orgnr`. No embedded `adresse`/`kontaktperson` JSON.
- `kontaktpersoner` - Contact persons (separate table, linked via `kontaktperson_kunde` junction)
- `vurderinger` - Condition assessments. Key columns: `id`, `tilfluktsrom` (text FK), `kontrolldato`, `vurderingsnummer`, `status`, `godkjent`, `tilstandsvurdering`, `kontaktperson_id`, `utførende`.
- `vurdering_avvik` - Deviations. Columns: `id`, `vurdering_id`, `checklist_item_id`, `emne`, `beskrivelse`, `photo_ids`. No `gruppe`/`punkt` columns.
